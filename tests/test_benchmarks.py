...
from chatterbox.trainers import ListTrainer, ChatterBoxCorpusTrainer, UbuntuCorpusTrainer
from chatterbox.logic import BestMatch
from chatterbox import comparisons, response_selection, utils
...
def get_chatterbox_corpus_trainer(chatbot):
    return ChatterBoxCorpusTrainer(
        chatbot,
        show_training_progress=False
    )
...
class SqlBenchmarkingTests(BenchmarkingMixin, ChatBotSQLTestCase):
    ...
    def test_get_response_after_chatterbox_corpus_training(self):
        """
        Test response time after training with the ChatterBox corpus.
        """
        trainer = get_chatterbox_corpus_trainer(self.chatbot)
        trainer.train('chatterbox.corpus')
...
class MongoBenchmarkingTests(BenchmarkingMixin, ChatBotMongoTestCase):
    ...
    def get_kwargs(self):
        kwargs = super().get_kwargs()
        kwargs['storage_adapter'] = 'chatterbox.storage.MongoDatabaseAdapter'
        return kwargs
