...
class CorpusLoadingTestCase(TestCase):
    ...
    def test_load_corpus_chinese(self):
        data_files = corpus.list_corpus_files('chatterbox.corpus.chinese')
    ...
    def test_load_corpus_english_greetings(self):
        data_files = corpus.list_corpus_files('chatterbox.corpus.english.greetings')
        ...
        self.assertIn('chatterbox_corpus/data/english/greetings.yml', file_path)
    ...
    def test_load_corpus_french(self):
        data_files = corpus.list_corpus_files('chatterbox.corpus.french')
        corpus_data = corpus.load_corpus(*data_files)

        self.assertTrue(len(list(corpus_data)))

    def test_load_corpus_german(self):
        data_files = corpus.list_corpus_files('chatterbox.corpus.german')
        corpus_data = corpus.load_corpus(*data_files)

        self.assertTrue(len(list(corpus_data)))

    def test_load_corpus_hindi(self):
        data_files = corpus.list_corpus_files('chatterbox.corpus.hindi')
        corpus_data = corpus.load_corpus(*data_files)

        self.assertTrue(len(list(corpus_data)))

    def test_load_corpus_indonesian(self):
        data_files = corpus.list_corpus_files('chatterbox.corpus.indonesian')
        corpus_data = corpus.load_corpus(*data_files)

        self.assertTrue(len(list(corpus_data)))

    def test_load_corpus_italian(self):
        data_files = corpus.list_corpus_files('chatterbox.corpus.italian')
        corpus_data = corpus.load_corpus(*data_files)

        self.assertTrue(len(list(corpus_data)))

    def test_load_corpus_marathi(self):
        data_files = corpus.list_corpus_files('chatterbox.corpus.marathi')
        corpus_data = corpus.load_corpus(*data_files)

        self.assertTrue(len(list(corpus_data)))

    def test_load_corpus_portuguese(self):
        data_files = corpus.list_corpus_files('chatterbox.corpus.portuguese')
        corpus_data = corpus.load_corpus(*data_files)

        self.assertTrue(len(list(corpus_data)))

    def test_load_corpus_russian(self):
        data_files = corpus.list_corpus_files('chatterbox.corpus.russian')
        corpus_data = corpus.load_corpus(*data_files)

        self.assertTrue(len(list(corpus_data)))

    def test_load_corpus_spanish(self):
        data_files = corpus.list_corpus_files('chatterbox.corpus.spanish')
        corpus_data = corpus.load_corpus(*data_files)

        self.assertTrue(len(list(corpus_data)))

    def test_load_corpus_telugu(self):
        data_files = corpus.list_corpus_files('chatterbox.corpus.telugu')
        corpus_data = corpus.load_corpus(*data_files)
...
class CorpusUtilsTestCase(TestCase):
    ...
    def test_load_corpus(self):
        ...
        """
        corpus_files = corpus.list_corpus_files('chatterbox.corpus')
