...
ChatterBox supports the ability to have multiple concurrent conversations.
...
The following example is taken from the Django ``ChatterBoxApiView`` built into ChatterBox.
...
.. literalinclude:: ../examples/django_app/example_app/views.py
   :language: python
   :pyobject: ChatterBoxApiView.post
   :dedent: 4
...
ChatterBox's statement objects represent either an input statement that the
chat bot has received from a user, or an output statement that the chat bot
has returned based on some input.

.. autoclass:: chatterbox.conversation.Statement
   :members:

   .. autoinstanceattribute:: chatterbox.conversation.Statement.confidence

      ChatterBox's logic adapters assign a confidence score to the statement
   ...
   .. autoinstanceattribute:: chatterbox.conversation.Statement.in_response_to
...
ChatterBox stores knowledge of conversations as statements. Each statement can have any
number of possible responses.

.. image:: _static/statement-response-relationship.svg
   :alt: ChatterBox statement-response relationship
...
.. image:: _static/statement-relationship.svg
   :alt: ChatterBox statement relationship
