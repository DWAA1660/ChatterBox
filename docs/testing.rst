...
ChatterBox's built in tests can be run using nose.
...
You can run all of ChatterBox's tests with a single command: ``tox``.
...
the ``ChatterBox`` repository it will run the following tests:

1. Tests for ChatterBox's core files.
2. Tests for ChatterBox's integration with multiple versions of Django.
3. Tests for each of ChatterBox's example files.
4. Tests to make sure ChatterBox's documentation builds.
