...
ChatterBox's :term:`adapters`. In this class, an input statement is
       ...
       logic_adapters=[
           {
               ...
               'statement_comparison_function': chatterbox.comparisons.LevenshteinDistance
               'response_selection_method': chatterbox.response_selection.get_first_response
           },
           {
...
ChatterBox has built in logging. You can enable ChatterBox's
...
ChatterBox uses adapter modules to control the behavior of specific types of tasks.
There are four distinct types of adapters that ChatterBox uses,
these are storage adapters and logic adapters.

Adapters types
--------------

1. Storage adapters - Provide an interface for ChatterBox to connect to various storage systems such as `MongoDB`_ or local file storage.
2. Logic adapters - Define the logic that ChatterBox uses to respond to input it receives.

Accessing the ChatBot instance
-------------------------------

When ChatterBox initializes each adapter, it sets an attribute named ``chatbot``.
