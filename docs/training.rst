...
ChatterBox includes tools that help simplify the process of training a chat bot instance.
ChatterBox's training process involves loading example dialog into the chat bot's database.
...
.. image:: _static/training-graph.svg
   :alt: ChatterBox training statement graph

Several training classes come built-in with ChatterBox. These utilities range from allowing
...
ChatterBox comes with training classes built in, or you can create your own
...
.. autofunction:: chatterbox.trainers.ListTrainer
...
For example, if you were to run bot of the following training calls, then the resulting chatterbox would respond to
...
.. autofunction:: chatterbox.trainers.ChatterBoxCorpusTrainer

ChatterBox comes with a corpus data and utility module that makes it easy to
...
It is also possible to import individual subsets of ChatterBox's corpus at once.
...
.. autofunction:: chatterbox.trainers.UbuntuCorpusTrainer
...
Your custom trainer should inherit `chatterbox.trainers.Trainer` class.
...
.. _`trainer classes on GitHub`: https://github.com/gunthercox/ChatterBox/blob/master/chatterbox/trainers.py
