...
asked about ChatterBox and chat bots in general.
...
examples of these. ChatterBox is designed to be agnostic to
the platform it is deployed on and it is very easy to get set up.

To run ChatterBox inside of a web application you just need a way
...
1. An example using Django: https://github.com/gunthercox/ChatterBox/tree/master/examples/django_app
2. An example using Flask: https://github.com/chamkank/flask-chatterbox/blob/master/app.py

Additional details and recommendations for configuring Django can be found
in the :ref:`Webservices` section of ChatterBox's Django documentation.

What kinds of machine learning does ChatterBox use?
---------------------------------------------------

In brief, ChatterBox uses a number of different machine learning techniques to
...
that are employed throughout ChatterBox's codebase.
...
Several logic adapters in ChatterBox use `naive Bayesian classification`_
